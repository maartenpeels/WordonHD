    ...
    implements FREFunction
{
    ...
    public FREObject call(FREContext frecontext, FREObject afreobject[])
    {
        ...
        {
            FREUtils.logEvent(frecontext, LogLevel.FATAL, "Unable to read the 'id' parameter. [Exception:(type:%s, method:%s)].", new Object[] {
                FREUtils.stripPackageFromClassName(afreobject.toString()), FREUtils.stripPackageFromClassName(StackTraceInfo.getCurrentClassName())
            });
            return FREUtils.createRuntimeException("ArgumentError", 0, "Unable to read the 'id' parameter on method '%s'.", new Object[] {
                FREUtils.stripPackageFromClassName(StackTraceInfo.getCurrentClassName())
            });
        }
        ((Tracker) (obj)).setAppId(afreobject);
        return null;
    }
}
