...
public class SetCustomMetric
    implements FREFunction
{

    public SetCustomMetric()
    {
    }
    ...
    public FREObject call(FREContext frecontext, FREObject afreobject[])
    {
        ...
        try
        {
            i = afreobject[1].getAsInt();
        }
        // Misplaced declaration of an exception variable
        catch (FREObject afreobject[])
        {
            FREUtils.logEvent(frecontext, LogLevel.FATAL, "Unable to read the 'index' parameter. [Exception:(type:%s, method:%s)].", new Object[] {
                FREUtils.stripPackageFromClassName(afreobject.toString()), FREUtils.stripPackageFromClassName(StackTraceInfo.getCurrentClassName())
            });
            return FREUtils.createRuntimeException("ArgumentError", 0, "Unable to read the 'index' parameter on method '%s'.", new Object[] {
                FREUtils.stripPackageFromClassName(StackTraceInfo.getCurrentClassName())
            });
        }
        try
        {
            j = afreobject[2].getAsInt();
        }
        // Misplaced declaration of an exception variable
        catch (FREObject afreobject[])
        {
        ...
        ((Tracker) (obj)).setCustomMetric(Integer.valueOf(i).intValue(), Long.valueOf(Integer.valueOf(j).intValue()));
        return null;
    }
}
