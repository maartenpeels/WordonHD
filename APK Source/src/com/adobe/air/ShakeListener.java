    ...
    implements SensorEventListener
{
    ...
    public void onSensorChanged(SensorEvent sensorevent)
    {
        ...
        {
            long l1 = mLastTime;
            if ((float)((Math.sqrt((sensorevent.values[0] - mLastX) * (sensorevent.values[0] - mLastX) + (sensorevent.values[1] - mLastY) * (sensorevent.values[1] - mLastY) + (sensorevent.values[2] - mLastZ) * (sensorevent.values[2] - mLastZ)) / (double)(l - l1)) * 10000D) > 1200F)
            {
                int i = mShakeCount + 1;
                mShakeCount = i;
