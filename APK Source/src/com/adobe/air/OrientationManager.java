    ...
    public int[] getSupportedOrientations()
    {
        if (android.os.Build.VERSION.SDK_INT <= 8)
        {
            ...
            if (mOsDefaultOrientation == 1)
            {
                ...
                ai[1] = EOrientation.ROTATED_RIGHT.ordinal();
                return ai;
            } else
            {
                ai[0] = EOrientation.DEFAULT.ordinal();
        ...
        {
            return (new int[] {
                EOrientation.DEFAULT.ordinal(), EOrientation.ROTATED_LEFT.ordinal(), EOrientation.ROTATED_RIGHT.ordinal(), EOrientation.UPSIDE_DOWN.ordinal()
            });
        }
    }
    ...
    public void onActivityCreated(Activity activity, AIRWindowSurfaceView airwindowsurfaceview)
    {
        ...
        mOrientationEventListner = new OrientationEventListener(m_activity.getApplicationContext(), 3) {
            ...
            public void onOrientationChanged(int i)
            {
                int j;
                int k;
                k = EOrientation.UNKNOWN.ordinal();
                if (i == -1)
                {
                    mDeviceOrientation = EOrientation.UNKNOWN.ordinal();
...
_L4:
                ...
                {
                    i = android.provider.Settings.System.getInt(m_activity.getContentResolver(), "accelerometer_rotation", 0);
                    if (m_activity != null && i != 0 && isReOrientingAllowed())
                    {
...
_L2:
        ...
        activity = AndroidActivityWrapper.GetAndroidActivityWrapper();
        if (activity.isScreenOn() && !activity.isScreenLocked())
        {
            if (!mFirstCreate)
            {
...
_L4:
        ...
        if (mNamespaceVersion >= 3.8F && i != j)
        {
            nativeOrientationChanged(j, i);
        }
        return;
_L2:
        if (i == EOrientation.DEFAULT.ordinal())
