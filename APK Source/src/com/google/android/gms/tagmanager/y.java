    ...
    implements aq
{
    ...
    public boolean bz(String s)
    {
        ...
        if (XN != null && XO != null)
        {
            try
            {
                s = (new StringBuilder()).append(XN).append("?").append(XO).append("=").append(URLEncoder.encode(s, "UTF-8")).toString();
                bh.y((new StringBuilder()).append("Sending wrapped url hit: ").append(s).toString());
            }
