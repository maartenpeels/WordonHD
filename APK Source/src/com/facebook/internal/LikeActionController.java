...
public class LikeActionController
{
    ...
    private static void broadcastAction(Context context1, LikeActionController likeactioncontroller, String s, Bundle bundle)
    {
        ...
        w.a(context1.getApplicationContext()).a(intent);
    }
...
_L3:
        context1 = deserializeFromJson(context1, s1);
_L8:
        Object obj;
        if (s != null)
        {
...
_L7:
    ...
    private static LikeActionController deserializeFromJson(Context context1, String s)
    {
        ...
        try
        {
            context1 = new LikeActionController(context1, Session.getActiveSession(), s.getString("object_id"));
            context1.likeCountStringWithLike = s.optString("like_count_string_with_like", null);
            context1.likeCountStringWithoutLike = s.optString("like_count_string_without_like", null);
            context1.socialSentenceWithLike = s.optString("social_sentence_with_like", null);
            context1.socialSentenceWithoutLike = s.optString("social_sentence_without_like", null);
            context1.isObjectLiked = s.optBoolean("is_object_liked");
            context1.unlikeToken = s.optString("unlike_token", null);
            String s1 = s.optString("pending_call_id", null);
            if (!Utility.isNullOrEmpty(s1))
            {
                context1.pendingCallId = UUID.fromString(s1);
            }
            s = s.optJSONObject("pending_call_analytics_bundle");
        }
        ...
        context1.pendingCallAnalyticsBundle = BundleJSONConverter.convertToBundle(s);
        return context1;
    }
    ...
    private boolean onActivityResult(int i, int j, Intent intent, UUID uuid)
    {
        ...
        uuid = PendingCallStore.getInstance().getPendingCallById(pendingCallId);
        if (uuid == null)
        {
            return false;
        } else
...
_L2:
        ...
        s = null;
_L5:
        Log.e(TAG, "Unable to serialize controller to disk", s1);
        if (s == null) goto _L2; else goto _L1
_L1:
        Utility.closeQuietly(s);
        return;
...
_L3:
    ...
    private void updateState(boolean flag, String s, String s1, String s2, String s3, String s4)
    {
        ...
        } else
        {
            ...
            unlikeToken = s4;
            serializeToDiskAsync(this);
            broadcastAction(context, this, "com.facebook.sdk.LikeActionController.UPDATED", null);
            return;
        }
    }
